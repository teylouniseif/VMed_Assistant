...
def answer_question_part(question, answer_text):
    ...
    '''
    #proxies = {}
    bertmodel=os.path.join(os.path.dirname(os.path.abspath(__file__)),'bertmodel')
    print("hello")
    print(bertmodel)
    tokenizer = BertTokenizer.from_pretrained(bertmodel)#'bert-large-uncased-whole-word-masking-finetuned-squad')      
    # ======== Tokenize ========
    ...
    del tokenizer
    model = BertForQuestionAnswering.from_pretrained(bertmodel)#'bert-large-uncased-whole-word-masking-finetuned-squad')
